...
/// </summary>
public sealed record CreateResult : CommandResultBase
{
    private readonly Guid? _aggregateId;


    private CreateResult(Guid aggregateId) =>
        _aggregateId = aggregateId;

    private CreateResult(DomainError error) : base(error)
    {
    }

    private CreateResult(Exception panicException) : base(panicException)
    {
    }
    ...
    /// Factory for success
    /// </summary>
    public static CreateResult Success(Guid aggregateId) => new(aggregateId);
    ...
    /// <returns></returns>
    public static CreateResult DomainFail(DomainError error) => new(error);
    ...
    /// <returns></returns>
    public static CreateResult PanicFail(Exception panicException) => new(panicException);
    ...
    /// <returns></returns>
    public static implicit operator bool(CreateResult result) => result.IsSuccess;
